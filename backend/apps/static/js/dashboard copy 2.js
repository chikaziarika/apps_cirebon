...
function showDrillDownTotalDI() {
    // 1. Gunakan API Bootstrap untuk berpindah ke tab "Data Daerah Irigasi"
    var tableTab = new bootstrap.Tab(document.querySelector('#table-tab'));
    tableTab.show();

    // 2. Scroll otomatis ke arah tabel agar user tahu konten sudah berubah
    document.getElementById('irigasiTable').scrollIntoView({ 
        behavior: 'smooth', 
        block: 'center' 
    });

    // 3. Optional: Berikan highlight sesaat pada tabel agar menarik perhatian
    const tableCard = document.querySelector('#table-pane .card');
    tableCard.style.transition = "all 0.5s";
    tableCard.style.ring = "4px solid #0d6efd";
    setTimeout(() => {
        tableCard.style.ring = "none";
    }, 1500);
}

// Tambahkan juga fungsi pendukung jika Bapak menggunakan GSAP untuk animasi
function backToRingkasan() {
    // Kembali ke tab utama
    var chartTab = new bootstrap.Tab(document.querySelector('#chart-tab'));
    chartTab.show();
    
    // Pastikan view ringkasan tampil (jika sebelumnya disembunyikan oleh drilldown lain)
    $("#view-ringkasan").show().css("opacity", 1);
    $("#view-drilldown, #view-detail-luas, #view-detail-sebaran, #view-detail-distribusi").hide();
}
